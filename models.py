    ...
    def build(self):
        ...
        if self.config.skip_doc_attention:
            ...
            if self.config.tree_percolation_levels == 1:
                ...
                sents_r_root = tf.concat([tf.tile(embeddings_root, [batch_l, 1, 1]), sents_r], 1)
                sents_c2 = tf.matmul(str_scores, sents_r_root)
                sents_output = LReLu(tf.tensordot(tf.concat([sents_r, sents_c2], 2), w_comb, [[2], [0]]) + b_comb)
            elif self.config.tree_percolation_levels == 2:
                sents_c = tf.matmul(str_scores_no_root, sents_sem)
                sents_r = LReLu(tf.tensordot(tf.concat([sents_sem, sents_c], 2), w_comb, [[2], [0]]) + b_comb)
                sents_c2 = tf.matmul(str_scores_no_root, sents_r)
                sents_r2 = LReLu(tf.tensordot(tf.concat([sents_r, sents_c2], 2), w_comb, [[2], [0]]) + b_comb)
                sents_r2_root = tf.concat([tf.tile(embeddings_root, [batch_l, 1, 1]), sents_r2], 1)
                sents_c3 = tf.matmul(str_scores, sents_r2_root)
                sents_output = LReLu(tf.tensordot(tf.concat([sents_r2, sents_c3], 2), w_comb, [[2], [0]]) + b_comb)
            else:
